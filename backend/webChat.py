...
PUBLICPATH = os.environ.get("PUBLICPATH")

temp_voice_dir = os.environ.get("TEMP_VOICE_DIR", "%s/audio" % (PUBLICPATH))
...
def analyze_emotions(text):
    ...
    for i, (full_tag, emotion_tag, following_text) in enumerate(emotion_segments, 1):
        ...
        try:
            ...
            prediction_result = {"label": "unknown", "confidence": 0.0}
        ...
        results.append(
            {
                ...
                "voice_file": os.path.join(
                    temp_voice_dir, f"part_{i}.{tts_engine.format}"
                ),
            }
        )
...
async def generate_voice_files(text_segments):
    ...
    for segment in text_segments:
        if segment["japanese_text"]:  # 确保有实际内容
            task = text_to_speech(segment["japanese_text"], segment["voice_file"])
            tasks.append(task)
...
def play_voice_files(text_segments):
    ...
    for segment in text_segments:
        if os.path.exists(segment["voice_file"]):
            ...
            print(
                f"预测情绪: {segment['predicted']} (置信度: {segment['confidence']:.2%})"
            )
