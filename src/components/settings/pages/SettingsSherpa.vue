      ...
      <div class="flex min-h-0 flex-col gap-6">
        ...
        <section class="config-section">
          ...
          <div class="model-list">
            ...
            <p v-if="availableModels.length === 0" class="no-models">{{ t('settings.sherpa.noModels') }}</p>
          </div>
          ...
          <div class="ref-audio-card" :class="{ active: config.ref_audio_path }">
            <div class="ref-audio-info">
              ...
              <div v-else class="ref-audio-detail">
                ...
                <span class="ref-audio-status">WAV / MP3 / OGG / FLAC</span>
              </div>
            </div>
            <div class="ref-audio-actions">
              <button class="btn-secondary" @click="pickRefAudio">
                <FolderOpen :size="16" />
                {{ config.ref_audio_path ? t('settings.sherpa.refChange') : t('settings.sherpa.refPick') }}
              </button>
              <button
                v-if="config.ref_audio_path"
                class="btn-secondary btn-danger"
                @click="config.ref_audio_path = ''; config.ref_text = ''"
              >
...
import {
  Cpu, Package, Settings, SlidersHorizontal, Wand2, Mic, Play, Square,
  FolderOpen, Download, Trash2, Loader2, X,
