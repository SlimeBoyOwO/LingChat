  ...
  <div class="h-full overflow-y-auto p-6">
    ...
    <section class="mb-6">
      ...
      <select
        ...
        class="shadow-glass focus:border-brand focus:ring-brand/20 mb-4 w-full rounded-lg border
          border-white/10 bg-white/10 px-3 py-2.5 text-sm text-sky-400 backdrop-blur-xl
          backdrop-saturate-150 transition-all duration-200 focus:ring-2 focus:outline-none"
      >
      ...
      <div v-if="activeProviderInfo" class="space-y-3">
        <div v-for="field in activeProviderInfo.config_fields" :key="field.key">
          ...
          <div v-if="field.key === 'model' && asrStore.models.length > 0" class="flex gap-2">
            ...
            <button
              ...
              class="rounded-lg border border-white/15 bg-white/5 px-3 text-white/60
        ...
        <div class="flex items-center gap-3">
          <button
            type="button"
            class="bg-brand rounded-lg px-4 py-2 text-sm text-white transition-colors duration-200
              hover:bg-[#0056b3]"
...
<script setup lang="ts">
  ...
  const activeModel = computed(() => {
    const id =
      localSettings.value.provider_configs[localSettings.value.active_provider]?.model ?? "";
