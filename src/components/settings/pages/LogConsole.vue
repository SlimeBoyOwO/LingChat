<template>
  ...
  <div
    ...
    class="flex max-h-[70dvh] min-h-0 flex-1 flex-col items-center justify-center gap-3.5 rounded-xl border border-dashed border-white/20 bg-black/40 px-4 py-8 text-white/55"
  >
    <PictureInPicture2 :size="36" />
    <div class="text-center text-sm leading-loose">{{ $t("settings.log.poppedOut") }}</div>
    <button
      class="cursor-pointer rounded-lg border-none bg-(--accent-color,#79d9ff) px-4 py-1.5 text-[13px] font-semibold text-[#0b2530] transition-all duration-200 hover:-translate-y-px hover:shadow-[0_4px_10px_rgba(121,217,255,0.4)]"
      @click="popoutWindow"
    >
  ...
  <div v-else class="flex h-full min-h-0 flex-col">
    ...
    <div class="mb-3 flex shrink-0 flex-wrap items-center justify-between gap-3">
      <div class="flex items-center gap-1.5">
        <button
          v-for="lvl in levels"
          :key="lvl.key"
          class="cursor-pointer rounded-md border border-transparent bg-[#e9ecef] px-2.5 py-[3px] text-[11px] font-semibold tracking-[0.3px] text-[#495057] transition-all duration-200 hover:-translate-y-px hover:bg-(--accent-color,#79d9ff) hover:text-white hover:shadow-[0_4px_10px_rgba(121,217,255,0.4)]"
          :class="
            isLevelVisible(lvl.key)
              ? `border-(--lvl-color)! bg-(--lvl-bg)! text-(--lvl-color)! hover:bg-(--lvl-color)! hover:text-white!`
              : ''
          "
          :style="{
            '--lvl-color': lvl.color,
    ...
    <div
      ref="logContainer"
      class="min-h-0 flex-1 scrollbar-thin overflow-x-hidden overflow-y-auto rounded-xl border border-white/10 bg-black/65 px-3 py-3 font-['Cascadia_Code','Fira_Code','JetBrains_Mono','Consolas',monospace] text-[13px] leading-[1.7] backdrop-blur-md"
      :class="standalone ? 'max-h-none' : 'max-h-[70dvh]'"
      :style="{ scrollbarColor: 'var(--accent-color, #79d9ff) transparent' }"
      @scroll="handleScroll"
    >
      <div v-if="filteredLogs.length === 0" class="flex flex-1 items-center justify-center py-10">
        <div class="text-center text-xl font-bold text-gray-100 opacity-60">
      ...
      <template v-for="(entry, _idx) in filteredLogs" :key="_idx">
        ...
        <div
          class="block rounded-sm py-px hover:bg-white/[0.04]"
          :class="{ 'py-0.5': uiStore.isNarrowScreen }"
        >
          <span class="mr-2.5 inline-block min-w-[88px] text-xs text-white/30 tabular-nums">{{
            entry.timestamp
          }}</span>
          <span
            class="mr-2.5 inline-block w-[38px] rounded-[3px] px-1 text-center text-[10px] leading-[18px] font-bold"
            :class="levelTagClass(entry.level)"
            >{{ entry.level }}</span
          >
          <span
            class="mr-2.5 inline text-xs wrap-anywhere text-white/40"
            :class="uiStore.isNarrowScreen ? 'after:content-none' : 'after:content-[\':\']'"
            >{{ entry.target }}</span
          >
          <span
            class="inline break-words wrap-anywhere whitespace-pre-wrap text-white/85"
      ...
      <div
        ...
        class="mt-3 border-t border-dashed border-yellow-500/30 pt-3 text-center text-sm text-yellow-400"
      >
