<template>
  <MenuPage>
      ...
      <div v-if="audioOutputSupported" class="flex w-full flex-col gap-2">
        <div class="flex w-full items-stretch gap-2">
          <select
            ...
            class="output-device-select min-w-0 flex-1 cursor-pointer appearance-none rounded-lg border border-white/20 bg-white/10 py-2 pr-8 pl-3 text-sm text-white transition-colors outline-none focus:border-(--accent-color)"
          ...
          <button
            ...
            type="button"
            class="inline-flex flex-none cursor-pointer items-center justify-center gap-1 rounded-lg border border-white/20 bg-white/10 px-3 py-2 text-sm text-white transition-colors hover:border-(--accent-color) hover:bg-white/20 active:bg-white/30"
    ...
    <MenuItem :title="$t('settings.sound.bgm.title')">
      ...
      <!-- 音乐控制台 -->
      <div
        class="flex gap-3 rounded-xl border border-white/10 bg-white/5 p-4 backdrop-blur-md max-[640px]:flex-col max-[640px]:gap-2"
      >
        <div
          class="flex w-[60%] items-center justify-between rounded-lg bg-black/20 px-3 py-2 text-sm font-medium text-gray-200 max-[640px]:w-full"
        >
      ...
      <div class="mt-4 mb-2 flex flex-wrap items-center gap-2">
        ...
        <div class="flex items-center gap-1">
          <input
            ...
            class="w-28 rounded-lg border border-white/15 bg-black/30 px-2 py-1 text-xs text-white focus:border-purple-400 focus:outline-none"
            @keyup.enter="handleCreateMusicCategory"
          />
          <button
            class="rounded-full border border-purple-400 bg-purple-500/80 px-2.5 py-1 text-xs font-semibold text-white hover:bg-purple-500"
        ...
        <!-- 刷新 -->
        <button
          class="rounded-full border border-white/20 bg-white/10 px-2.5 py-1 text-xs font-semibold text-white/70 hover:bg-white/20"
      ...
      <!-- 音乐列表 -->
      <div
        class="mt-4 flex flex-col overflow-hidden rounded-xl border border-white/10 bg-black/20 backdrop-blur-sm"
      >
        ...
        <div v-else class="custom-scrollbar max-h-52 space-y-1 overflow-y-auto p-1.5">
          <div
            ...
            class="group flex cursor-pointer items-center justify-between rounded-lg px-3 py-2.5 transition-all duration-200 hover:bg-white/10"
            :class="{ 'bg-purple-500/20 text-purple-300': currentMusicName === music.name }"
          >
            <div
              class="flex flex-1 items-center gap-2 overflow-hidden pr-2 text-sm font-medium text-ellipsis whitespace-nowrap"
            >
              <button
            ...
            <button
              ...
              class="rounded-md bg-red-500/10 p-1.5 text-red-400 opacity-0 transition-opacity duration-200 group-hover:opacity-100 hover:bg-red-500/80 hover:text-white"
    ...
    <MenuItem :title="$t('settings.sound.ambient.title')">
      ...
      <div
        class="flex flex-col overflow-hidden rounded-xl border border-white/10 bg-black/20 backdrop-blur-sm"
      >
        ...
        <div v-else class="custom-scrollbar max-h-32 space-y-1 overflow-y-auto p-1.5">
          <div
            ...
            :key="ambient.url"
            class="group flex items-center gap-2 rounded-lg bg-white/5 px-3 py-2 transition-all duration-200 hover:bg-white/10"
          >
            ...
            <button
              ...
              class="rounded bg-teal-500/20 px-2 py-0.5 text-xs text-teal-300 opacity-70 transition-opacity hover:bg-teal-500/40 hover:opacity-100"
            ...
            <button
              ...
              class="rounded-md bg-red-500/10 p-1.5 text-red-400 opacity-0 transition-opacity group-hover:opacity-100 hover:bg-red-500/80 hover:text-white"
              :title="$t('settings.sound.common.delete')"
            >
              <Trash2 :size="14" />
            </button>
      ...
      >
        ...
        <div v-else class="custom-scrollbar max-h-48 space-y-1 overflow-y-auto p-1.5">
          <div
            ...
            :key="track.id"
            class="group flex flex-col gap-1.5 rounded-lg bg-cyan-500/10 px-3 py-2 transition-all duration-200"
          >
            <!-- 轨道名 + 控制按钮 -->
            <div class="flex items-center gap-2">
              ...
              <button
                ...
                class="rounded bg-red-500/10 p-1 text-red-400 opacity-0 transition-opacity group-hover:opacity-100 hover:bg-red-500/40 hover:text-white"
...
const uploadAmbientFiles = async () => {
  ...
  try {
    ...
    const translated =
      rawMsg === "MUSIC_INVALID_FORMAT" ? t("ui.musicImport.errors.MUSIC_INVALID_FORMAT") : rawMsg;
...
const removeAmbientFile = async (ambient: AmbientItem) => {
  if (
    !(await dialogStore.confirm(t("settings.sound.ambient.confirmDelete", { name: ambient.name })))
  )
    return;
  try {
