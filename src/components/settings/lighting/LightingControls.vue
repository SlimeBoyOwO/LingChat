<template>
  ...
  >
    ...
    >
      ...
      <div
        v-if="sec.pad"
        class="relative h-28 w-full cursor-crosshair touch-none overflow-hidden rounded-lg border
          border-white/15 select-none"
        :style="padStyle"
      ...
      >
        <div
          class="pointer-events-none absolute h-5 w-5 -translate-x-1/2 -translate-y-1/2 rounded-full
            border-2 border-white bg-amber-300 shadow-[0_0_14px_4px_rgba(252,211,77,0.8)]"
      ...
      <div v-for="sel in sec.selects" :key="sel.path" class="flex items-center gap-2">
        ...
        <select
          class="min-w-0 flex-1 rounded-lg border border-white/10 bg-black/40 px-2 py-1 text-[11px]
            text-white focus:border-amber-400/60 focus:outline-none"
