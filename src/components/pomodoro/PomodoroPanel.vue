<template>
  <div class="flex flex-col gap-3">
    ...
    >
      <div
        v-if="enabled"
        class="box-border flex w-65 flex-col items-center rounded-3xl border border-white/10 bg-[#12121c]/75 p-6 text-white shadow-[0_8px_32px_rgba(0,0,0,0.4)] backdrop-blur-[20px]"
      >
        <div class="relative mb-6 outline-none">
          <div class="relative h-45 w-45 border-none outline-none">
            ...
            >
              ...
              <circle
                class="stroke-round fill-none stroke-[url(#gradient-ring)] stroke-4 drop-shadow-[0_0_4px_rgba(79,172,254,0.5)] transition-[stroke-dashoffset] duration-1000 ease-linear"
                cx="50"
                cy="50"
                r="45"
                :style="progressStyle"
            ...
            >
              <div
                class="group pointer-events-auto mb-1 flex h-6 cursor-pointer items-center justify-center"
                @click="startEditLabel"
                :title="$t('ui.pomodoro.editName')"
              >
                <span
                  v-if="!editingLabel"
                  class="text-base font-medium tracking-wide opacity-90 transition-colors group-hover:text-[#4facfe]"
                >
                  {{ workLabel }}
                </span>
                <input
                  ...
                  class="w-30 border-0 border-b border-[#4facfe] bg-transparent p-0 text-center text-base text-white outline-none focus:ring-0"
              ...
              <div
                class="my-1 text-5xl leading-none font-bold tabular-nums drop-shadow-[0_4px_12px_rgba(0,0,0,0.3)]"
              >
                {{ minutes }}:{{ seconds }}
              </div>
        ...
        <div class="mb-6 flex w-40 items-center justify-between">
          <div
            class="flex h-12 w-12 cursor-pointer items-center justify-center rounded-full bg-white/10 text-white transition-all duration-200 hover:scale-105 hover:bg-white/20 active:scale-95"
        ...
        <div class="flex w-full justify-between border-t border-white/10 pt-4">
          <div class="flex flex-1 flex-col items-center">
            ...
            <div class="relative flex h-6 items-center justify-center">
              <input
                type="number"
                class="no-spin w-8 appearance-none border-none bg-transparent p-0 text-right text-[15px] font-medium text-white outline-none"
              ...
              <div class="flex h-full flex-col justify-center gap-0.5">
                <div
                  class="flex h-2.5 cursor-pointer items-center justify-center opacity-60 transition-transform hover:opacity-100 active:scale-90"
                  @click="adjustWork(1)"
                >
