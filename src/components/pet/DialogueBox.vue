<template>
  <div
    ...
    class="relative z-30 flex w-full cursor-pointer items-center justify-center transition-all
      duration-300 ease-out"
    :class="isVisible ? 'translate-y-0 opacity-100' : 'pointer-events-none translate-y-2 opacity-0'"
  >
    <div
      ref="bubbleRef"
      class="hover:-translate-y-0.2 relative w-[85%] rounded-[calc(20px*var(--pet-ui-scale,1))]
        border border-white/10 bg-neutral-950/50 px-[calc(18px*var(--pet-ui-scale,1))]
        py-[calc(6px*var(--pet-ui-scale,1))] text-white backdrop-blur-xl backdrop-saturate-200
        transition-all duration-300 [text-shadow:0_1px_4px_rgba(0,0,0,0.5)] hover:scale-[1.02]
        hover:border-white/20 hover:bg-neutral-950/65"
      :style="{ maxHeight: `calc(var(--dialog-h) - 20px)` }"
    >
      <div
        class="absolute -bottom-2.5 left-1/2 h-0 w-0 -translate-x-1/2 border-r-10 border-l-10
          border-t-white/10 border-r-transparent border-l-transparent drop-shadow-md"
      ></div>
      <div
        class="absolute -bottom-2 left-1/2 h-0 w-0 -translate-x-1/2 border-t-8 border-r-8 border-l-8
          border-t-white/8 border-r-transparent border-l-transparent"
      ></div>
      ...
      <div class="relative overflow-hidden">
        <Transition name="emotion-slide">
          <div
            ...
            class="mb-0.5 inline-block max-w-full truncate text-[calc(12px*var(--pet-ui-scale,1))]
              font-semibold tracking-wider text-cyan-400 italic
              drop-shadow-[0_1px_4px_rgba(0,176,255,0.5)]"
          >
            {{ characterEmotion }}
          </div>
        </Transition>
      ...
      <div
        ref="textareaRef"
        class="dialog-text-lock overflow-y-auto pb-[0.4em] text-[calc(15px*var(--pet-ui-scale,1))]
          leading-snug font-medium break-all whitespace-pre-line
