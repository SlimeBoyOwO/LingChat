<template>
  ...
  <div class="workshop-page">
    ...
    <div class="relative z-10 flex h-full w-full flex-col gap-4 p-4 md:p-8">
      ...
      <div
        class="custom-scrollbar min-h-0 flex-1 overflow-y-auto rounded-2xl border border-white/10 bg-white/8 p-4 backdrop-blur-2xl md:p-6"
      >
        <!-- Toolbar: category filter + sort toggle -->
        <div class="mb-5 flex shrink-0 flex-wrap items-center justify-between gap-2">
          <div class="flex flex-wrap items-center gap-1.5">
            <button
              class="cursor-pointer rounded-md border border-transparent bg-white/6 px-3 py-1 text-[13px] font-semibold tracking-[0.3px] text-white/50 transition-all duration-200 ease-in-out hover:bg-white/12 hover:text-white/80 [&.active]:border-(--cat-color,#79d9ff) [&.active]:bg-(--cat-bg,rgba(121,217,255,0.15)) [&.active]:text-(--cat-color,#79d9ff) [&.active]:hover:bg-(--cat-color,#79d9ff) [&.active]:hover:text-white"
          ...
          <div class="flex items-center gap-1 rounded-lg bg-white/5 p-0.5">
            <button
              class="cursor-pointer rounded-md border-none bg-transparent px-2.5 py-0.75 text-xs font-semibold text-white/40 transition-all duration-200 ease-in-out hover:text-white/70 [&.active]:bg-white/10 [&.active]:text-white"
        ...
        <template v-else>
          ...
          <div class="grid w-full grid-cols-1 gap-5 xl:grid-cols-2">
            <div
              ...
              class="group relative flex cursor-pointer items-start rounded-2xl border border-white/10 bg-white/10 p-5 backdrop-blur-xl transition-all duration-300 hover:-translate-y-0.5 hover:border-white/20 hover:shadow-xl hover:shadow-white/5"
              @click="openDiscussion(discussion.html_url)"
            >
              <!-- Top-left: category icon -->
              <div
                v-if="getCornerIcon(discussion.category.name)"
                class="text-brand absolute -top-2 -left-2 z-10 flex h-6 w-6 -rotate-18 items-center justify-center rounded-full shadow-md"
              >
