  ...
  <div class="workshop-page">
    ...
    <div class="relative z-10 flex h-full w-full flex-col gap-4 p-4 md:p-8">
      ...
      <div class="flex shrink-0 items-center justify-between gap-4">
        <button
          class="inline-flex shrink-0 items-center gap-2 rounded-xl border border-white/15
            bg-white/8 px-4 py-2 text-[0.9rem] text-white/80 backdrop-blur-xl transition-all
            duration-200 hover:bg-white/15 hover:text-white"
          @click="goBack"
        >
          ← {{ t("views.menu.back") }}
        </button>
        <h1
          class="flex-1 truncate text-center text-2xl font-bold text-white
            drop-shadow-[0_2px_8px_rgba(0,0,0,0.6)] md:text-3xl"
        >
      ...
      <div
        class="custom-scrollbar min-h-0 flex-1 overflow-y-auto rounded-2xl border border-white/10
          bg-white/8 p-4 backdrop-blur-2xl md:p-6"
      >
        <!-- Toolbar: category filter + sort toggle -->
        <div class="mb-5 flex shrink-0 flex-wrap items-center justify-between gap-2">
          <div class="flex flex-wrap items-center gap-1.5">
            <button
              class="cursor-pointer rounded-md border border-transparent bg-white/6 px-3 py-1
                text-[13px] font-semibold tracking-[0.3px] text-white/50 transition-all duration-200
                ease-in-out hover:bg-white/12 hover:text-white/80
                ...
                [&.active]:hover:bg-[color:var(--cat-color,#79d9ff)] [&.active]:hover:text-white"
          ...
          <div class="flex items-center gap-1 rounded-lg bg-white/5 p-0.5">
            <button
              class="cursor-pointer rounded-md border-none bg-transparent px-2.5 py-[3px] text-xs
                font-semibold text-white/40 transition-all duration-200 ease-in-out
                hover:text-white/70 [&.active]:bg-white/10 [&.active]:text-white"
        ...
        <div v-else-if="error" class="flex flex-col items-center justify-center gap-3 py-12">
          ...
          <button
            class="rounded-lg border border-white/10 bg-white/10 px-5 py-2 text-white
        ...
        <template v-else>
          ...
          <div
            ...
            class="mb-5 flex items-center gap-3 rounded-xl border border-yellow-500/25
              bg-yellow-500/10 px-5 py-3 text-sm text-yellow-200/80"
          >
          ...
          <div class="grid w-full grid-cols-1 gap-5 xl:grid-cols-2">
            <div
              ...
              class="group relative flex cursor-pointer items-start rounded-2xl border
                border-white/10 bg-white/10 p-5 backdrop-blur-xl transition-all duration-300
                hover:-translate-y-0.5 hover:border-white/20 hover:shadow-xl hover:shadow-white/5"
              @click="openDiscussion(discussion.html_url)"
            >
              <!-- Top-left: category icon -->
              <div
                v-if="getCornerIcon(discussion.category.name)"
                class="text-brand absolute -top-2 -left-2 z-10 flex h-6 w-6 -rotate-18 items-center
              ...
              <button
                class="absolute top-3 right-3 z-10 rounded-full bg-white/5 p-1.5 text-white/40
              ...
              >
                <div
                  class="h-28 w-28 shrink-0 overflow-hidden rounded-full border-2 border-white/20
                    shadow-lg"
                >
                  <img
                  ...
                  <div v-else class="flex h-full w-full items-center justify-center bg-white/5">
                    <img
                      ...
                      alt="Logo"
                      class="h-full w-full scale-130 -rotate-20 object-contain opacity-100"
                    />
                  </div>
                </div>
                <!-- Category badge -->
                <span
                  class="rounded-full border px-3 py-0.5 text-center text-sm leading-5 font-medium"
                  :style="{
              ...
              <div class="flex h-full min-w-0 flex-1 flex-col py-0.5 pl-4">
                <!-- Title -->
                <h3 class="mb-2 line-clamp-2 text-xl leading-7 font-bold text-white">
                  {{ discussion.title }}
                </h3>
                ...
                <!-- Footer: meta info -->
                <div
                  class="flex items-center gap-4 border-t border-white/5 pt-2.5 text-xs
                    text-white/35"
                >
                  <!-- Upvotes -->
                  <span
        ...
        <div v-if="totalPages > 1" class="mt-2 flex w-full items-center justify-between px-3 py-2">
          <button
            class="rounded-lg border-none bg-white/8 px-5 py-2 text-base font-medium text-white/60
        ...
        <div v-if="!loading && !error" class="mt-6 flex justify-center">
          <button
            class="rounded-lg border border-white/5 bg-white/5 px-5 py-1.5 text-sm text-white/40
              transition-all hover:border-white/15 hover:bg-white/10 hover:text-white/70"
            @click="load"
          >
