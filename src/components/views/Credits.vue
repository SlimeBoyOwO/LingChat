<template>
  ...
  <div
    class="relative flex h-full w-full items-center justify-center overflow-hidden bg-[#0a0a0c] font-['Noto_Sans_SC',sans-serif] text-white"
  >
    ...
    <div
      v-if="!isStarted"
      class="absolute inset-0 z-10 flex cursor-pointer items-center justify-center bg-[#0a0a0c]"
      @click="startCredits"
    >
      <!-- 右上角ESC退出提示 -->
      <div class="absolute top-6 right-6 text-sm tracking-wider text-white opacity-50">
        {{ $t("views.credits.escExit") }}
      </div>
      <div class="animate-[pulse_3s_infinite] text-center text-white">
        <p class="mb-2 text-[2.2em] font-light tracking-[2px]">
          {{ $t("views.credits.letterTitle") }}
        </p>
        <span class="text-[1.1em] tracking-[4px] uppercase opacity-70">A Letter For LingChat❤</span>
      </div>
    </div>
      ...
      >
        ...
        <div class="mb-20 flex w-full flex-col items-center">
          <img
            ...
            class="mx-auto mb-6 w-1/2 max-w-100 object-contain"
            alt="Logo"
          />
          <h1 class="text-[3.5em] font-normal tracking-[5px] text-[#00e5ff]">
            {{ $t("views.credits.heading") }}
          </h1>
          <p class="mt-2 text-[1.2em] tracking-[8px] text-[#ebfafb] uppercase opacity-70">
            CREDITS
          </p>
        </div>
        ...
        >
          ...
          <!-- 标准双语名单 (两列对齐) -->
          <div v-else-if="section.layout === 'normal'" class="mb-15 flex flex-col items-center">
            ...
            <div
              ...
              :key="i"
              class="mb-2 grid w-[27%] min-w-70 grid-cols-2 items-center justify-items-center"
            >
              <p class="text-[1.5em] leading-[1.8] font-light whitespace-nowrap">{{ item.name }}</p>
              <p
                class="pl-4 text-left text-[1em] leading-[1.8] font-light whitespace-nowrap opacity-80"
              >
                {{ item.enName }}
              </p>
            </div>
          ...
          <div
            ...
            class="mb-15 flex w-full flex-col items-center"
          >
            <h2 class="mb-2 text-[2.2em] font-light text-[#00e5ff]">{{ sectionTitle(section) }}</h2>
            <p class="mb-6.25 text-[1em] text-white opacity-60">{{ sectionSubtitle(section) }}</p>
            <div class="grid w-[60%] grid-cols-2 justify-items-center gap-x-12 gap-y-4">
              <div
                v-for="(item, i) in section.items"
                :key="i"
          ...
          <!-- 孤立文本 (如赞助者们等结尾长串) -->
          <div v-else-if="section.layout === 'single'" class="mb-4 flex flex-col items-center">
            <p class="text-[1.5em] leading-[1.8] font-light">{{ sectionTitle(section) }}</p>
            <p class="text-[1em] leading-[1.8] font-light opacity-80">
              {{ sectionSubtitle(section) }}
            </p>
          </div>

          <!-- 特殊结尾 (还有...你) -->
          <div v-else-if="section.layout === 'special'" class="mb-15 flex flex-col items-center">
            <div class="h-[60dvh]"></div>
            <h2 class="mb-2 text-[2.2em] font-light text-[#00e5ff]">{{ sectionTitle(section) }}</h2>
            <p class="mb-20 text-[1em] text-white opacity-60">{{ sectionSubtitle(section) }}</p>
            <div class="h-[60dvh]"></div>
