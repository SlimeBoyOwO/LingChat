<template>
  ...
  -->
  <div class="editor-root relative h-full w-full overflow-hidden">
    <div
      class="bg-layer"
      :style="{
    ...
    >
      <div class="bg-image" :style="{ backgroundImage: `url(${bgImageSrc})` }"></div>
      <div class="bg-dim-mask"></div>
    </div>

    <EditorHeader @playtest="playtest" @toggle-shortcut-help="shortcutHelp = true" @leave="leave" />

    <!-- 试玩前置条件不满足时的常驻提示。等作者点了「试玩」才报，他会先对着
         一个卡住不动的画面困惑一阵 —— 那正是这条横幅要省掉的时间。 -->
    <div
      v-if="store.detail && store.readiness && !store.readiness.ok"
      class="mx-5 mb-2 flex items-center gap-2.5 rounded-lg border border-amber-300/30 bg-amber-300/10 px-3 py-1.75 text-[0.76rem] leading-[1.7] text-amber-100/90"
    >
      <span
    ...
    <div class="relative h-[calc(100%-5.5rem)] min-h-0">
