// Shared driver for score-JSON songs: tempo map, chart generation and PCM synthesis.
// Score shape: { ppq, endTick, tempos: [[tick, µsPerBeat]], meters: [[tick, numerator, denominator]],
//   melody/harmony/bass: [[startTick, lengthTicks, midi, velocity]] } — ticks are relative to the first note.
import { frequency } from "./harmony.js";

export function createScoreSong(score) {
  const segments = [];
  for (const [tick, tempo] of score.tempos) {
    ...
    segments.push({
      tick,
      seconds: previous
        ? previous.seconds + ((tick - previous.tick) / score.ppq) * previous.beat
        : 0,
      beat: tempo / 1000000,
    });
  }
  ...
  const beat = segments[0].beat;
  const bpm = Math.round(60 / beat);
  const bpmLabel = score.tempos.length > 1 ? `${bpm} · 变速` : undefined;
  const countIn = 4 * beat;
  const duration = countIn + secondsAt(score.endTick) + 1.8;
