<template>
  <Transition name="modal">
    ...
    >
      ...
      >
        <!-- ─── 头部 ─── -->
        <div class="flex items-center gap-3 border-b border-white/10 bg-white/10 p-5">
          <div class="flex h-10 w-10 items-center justify-center rounded-xl" :class="headerIconBg">
            <Wifi :size="20" class="text-white" />
          </div>
          <div class="flex-1">
            <h2 class="text-lg leading-none font-bold text-white">{{ dialogTitle }}</h2>
            <p class="mt-1 text-xs text-white/50">{{ dialogSubtitle }}</p>
          </div>
          <button
            @click="emit('close')"
        ...
        <div class="flex-1 space-y-4 overflow-y-auto p-5">
          ...
          <div v-if="view === 'device-list' && phase === 'scanning'" class="py-8 text-center">
            ...
            <p class="mt-1 text-xs text-white/40">
              {{ $t("ui.lanSync.serverPort", { port: serverPort }) }}
            </p>
          </div>
          ...
          <div v-if="view === 'device-list' && phase !== 'scanning'" class="space-y-3">
            <div class="flex items-center justify-between">
              <span class="text-xs text-white/50">{{
                $t("ui.lanSync.peersFound", { count: peers.length })
              }}</span>
              <button
                @click="emit('rescan')"
            ...
            <div v-if="peers.length === 0" class="py-6 text-center text-sm text-white/40">
              <p>{{ $t("ui.lanSync.noPeers") }}</p>
              <p class="mt-1 text-xs">{{ $t("ui.lanSync.noPeersHint") }}</p>
            </div>
            ...
            >
              <div class="flex items-center justify-between">
                <div class="space-y-0.5">
                  <p class="text-sm font-bold text-white">{{ peer.deviceName }}</p>
                  <p class="text-xs text-white/40">{{ peer.host }}:{{ peer.port }}</p>
                  <p class="text-xs text-white/30">
                    {{
                      $t("ui.lanSync.peerInfo", {
                        version: peer.dataVersion,
                        count: peer.fileCount,
                      })
                    }}
                  </p>
                </div>
          ...
          <div v-if="view === 'sync-plan' && syncPlan" class="space-y-4">
            <section>
              <h3 class="mb-3 flex items-center gap-2 text-sm font-bold text-white">
                ...
                {{ $t("ui.lanSync.overview") }}
              </h3>
              <div class="space-y-1.5 rounded-xl bg-white/5 p-3 text-xs text-white/70">
                <p>
                  <span class="text-white/50">{{ $t("ui.lanSync.directionLabel") }}</span>
                  {{
                    syncPlan.direction === "pull"
                      ? $t("ui.lanSync.directionPull")
                      : $t("ui.lanSync.directionPush")
                  }}
                </p>
                <p>
                ...
                <p>
                  ...
                  {{
                    $t("ui.lanSync.transferSummary", {
                      count: syncPlan.filesToTransfer.length,
                      size: formatBytes(syncPlan.totalBytes),
                    })
                  }}
                </p>
            ...
            <section v-if="syncPlan.filesToTransfer.length > 0">
              <h3 class="mb-3 flex items-center gap-2 text-sm font-bold text-white">
                ...
                {{ $t("ui.lanSync.fileList") }}
              </h3>
              <div class="max-h-48 space-y-1 overflow-y-auto rounded-xl bg-white/5 p-3 text-xs">
                <p
                ...
                <p v-if="syncPlan.filesToTransfer.length > 50" class="text-center text-white/30">
          ...
          <div v-if="view === 'progress'" class="space-y-4 py-4 text-center">
            ...
            <p class="text-sm text-white/80">
              {{ progress.message || $t("ui.lanSync.transferring") }}
            </p>
            <p v-if="progress.currentFile" class="truncate px-4 text-xs text-white/40">
          ...
          <div v-if="view === 'result' && lastResult" class="space-y-3 py-4 text-center">
            <div
            ...
            >
              <Check v-if="lastResult.success" class="h-7 w-7 text-emerald-400" />
              <X v-else class="h-7 w-7 text-red-400" />
            </div>
            <p v-if="lastResult.success" class="text-lg font-bold text-emerald-400">
              {{ $t("ui.lanSync.syncComplete") }}
            </p>
            <p v-else class="text-lg font-bold text-red-400">{{ $t("ui.lanSync.syncFailed") }}</p>
            <p class="text-xs text-white/50">{{ lastResult.message }}</p>
            <div v-if="lastResult.success" class="space-y-0.5 text-xs text-white/40">
              ...
              <p>
                {{
                  $t("ui.lanSync.bytesTransferred", {
                    size: formatBytes(lastResult.bytesTransferred),
                  })
                }}
              </p>
            </div>
          ...
          <div v-if="view === 'result' && !lastResult" class="space-y-3 py-4 text-center">
            <div
              class="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-red-500/20"
            >
              <X class="h-7 w-7 text-red-400" />
            </div>
            <p class="text-lg font-bold text-red-400">{{ $t("ui.lanSync.syncFailed") }}</p>
            <p class="text-xs text-white/50">{{ errorMessage || $t("ui.lanSync.genericError") }}</p>
          </div>

          <!-- 通用错误（非 result 视图时显示） -->
          <div v-if="phase === 'error' && view !== 'result'" class="space-y-2 py-6 text-center">
            <div
              class="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-red-500/20"
            >
              <X class="h-7 w-7 text-red-400" />
            </div>
            <p class="font-bold text-red-400">{{ $t("ui.lanSync.errorTitle") }}</p>
            <p class="text-xs text-white/50">{{ errorMessage }}</p>
          </div>
        </div>

        <!-- ─── 按钮区 ─── -->
        <div class="shrink-0 space-y-2 p-5 pt-0">
          <button
          ...
          <p v-if="view === 'progress'" class="text-center text-xs text-white/30">
