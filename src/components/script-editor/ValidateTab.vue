  ...
  <MenuPage>
    <MenuItem :title="t('scriptEditor.validate.menuTitle')">
      ...
      <div class="mb-3 flex flex-wrap items-center gap-2">
        ...
        <span v-if="store.report" class="text-[0.78rem] text-white/50 [&_b]:font-semibold">
      ...
      <p v-if="!store.report" class="py-8 text-center text-[0.85rem] text-white/45">
      ...
      <template v-else>
        ...
        >
          ...
          >
            ...
            <span class="ml-auto flex gap-[0.6rem] text-[0.7rem] [&_b]:font-semibold">
              <b v-if="store.diagnosticsByChapter[c.id]?.errors" class="text-red-300"
                >{{ store.diagnosticsByChapter[c.id].errors }}
                {{ t("scriptEditor.validate.errors") }}</b
              >
              <b v-if="store.diagnosticsByChapter[c.id]?.warns" class="text-amber-300"
                >{{ store.diagnosticsByChapter[c.id].warns }}
                {{ t("scriptEditor.validate.warns") }}</b
              >
              <b v-if="store.diagnosticsByChapter[c.id]?.infos" class="text-white/50"
                >{{ store.diagnosticsByChapter[c.id].infos }}
                {{ t("scriptEditor.validate.infos") }}</b
              >
              <span v-if="!chapterHas(c.id)" class="text-green-300">{{
                t("scriptEditor.validate.passed")
              }}</span>
            </span>
            <button
          ...
          >
            ...
            <span
              ...
              class="text-brand shrink-0 text-[0.68rem] whitespace-nowrap opacity-70"
