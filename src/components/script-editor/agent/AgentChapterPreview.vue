...
const props = defineProps<{ scriptKey: string | null }>();
...
<template>
  ...
  <Teleport to="#app">
    ...
    >
      <div
        class="flex h-[min(78dvh,760px)] w-[min(1080px,94vw)] flex-col overflow-hidden rounded-xl border border-white/12.5 bg-[rgba(12,20,30,0.9)] shadow-[0_8px_32px_rgba(0,0,0,0.45),inset_0_1px_1px_rgba(255,255,255,0.06)] backdrop-blur-lg backdrop-saturate-[1.4]"
      >
        <div class="border-brand flex items-center gap-2 border-b-2 px-4 py-2.5">
          <h4 class="truncate text-[0.95rem] font-semibold text-white">
            {{
              scriptKey
          ...
          >
          <span
            class="ml-auto shrink-0 rounded-full border border-white/12 bg-white/5 px-2 py-[1px] text-[0.64rem] text-white/45"
          >
            {{ t("scriptEditor.agentScriptPreview.readOnly") }}
          </span>
          <button
            class="hover:text-brand shrink-0 cursor-pointer px-1 text-[0.85rem] text-white/50 transition-all duration-200 hover:rotate-90"
            @click="emit('close')"
          >
            ✕
          </button>
        ...
        <div v-else class="flex min-h-0 flex-1">
          ...
          <!-- 右：已落盘章节列表 -->
          <aside class="flex min-h-0 w-[210px] shrink-0 flex-col border-l border-white/10">
            ...
            <div class="flex min-h-0 flex-1 flex-col gap-1 overflow-y-auto px-2 pb-3">
              ...
              >
                ...
                <div class="mt-0.5 text-[0.64rem] text-white/35">
                  {{
                    t("scriptEditor.agentScriptPreview.eventCount", {
                      count: c.eventCount,
                    })
                  }}
                </div>
