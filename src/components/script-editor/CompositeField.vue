  ...
  <div class="rounded-xl border border-white/10 bg-black/15 p-3">
    ...
    <template v-if="field.kind === 'choice_options'">
      <div v-for="(opt, i) in rows" :key="i" class="mb-2 rounded-lg bg-white/6 p-2.5 last:mb-0">
        ...
        <div class="mt-1.5 ml-6 flex flex-wrap items-center gap-x-3 gap-y-1">
          <button
            v-if="!hasAddLine(i)"
          ...
          <button class="text-brand text-xs hover:underline" @click="addAction(i, 'set_var')">
            ＋ {{ t("scriptEditor.composite.addVariable") }}
          </button>
          <button
            v-if="!conditionOpen(i)"
            class="hover:text-brand text-xs text-white/40 transition-all"
