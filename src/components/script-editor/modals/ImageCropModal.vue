<template>
  ...
  <Teleport to="#app">
    ...
    >
      <div
        class="flex w-[min(760px,92vw)] flex-col overflow-hidden rounded-xl border border-white/12.5 bg-[rgba(12,20,30,0.94)] shadow-[0_8px_32px_rgba(0,0,0,0.45)]"
      >
        <div class="flex items-center justify-between gap-4 border-b border-white/10 px-5 py-3">
          <h4 class="font-semibold text-white">{{ t("scriptEditor.imageCrop.title") }}</h4>
          <span class="text-[0.7rem] text-white/45">{{ t("scriptEditor.imageCrop.hint") }}</span>
        </div>

        <div class="relative max-h-[52dvh] min-h-[300px] overflow-hidden bg-black/70">
          <img
            ref="imgEl"
            :src="imgSrc"
            class="max-w-full"
        ...
        <div class="flex items-center justify-end gap-2 px-5 py-3">
          <button
            class="rounded-lg border border-white/10 bg-white/6 px-4 py-1.5 text-[0.8rem] text-white/70 transition-colors hover:bg-white/[0.12] hover:text-white"
            @click="emit('cancel')"
          >
            {{ t("scriptEditor.imageCrop.cancel") }}
          </button>
          <button
            class="border-brand/45 bg-brand/14 text-brand hover:bg-brand/24 rounded-lg border px-4 py-1.5 text-[0.8rem] transition-colors"
            @click="confirm"
          >
