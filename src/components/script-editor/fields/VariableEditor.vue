<template>
  <div>
    ...
    <p v-if="displayHint" class="mb-1.5 text-xs text-white/35">
      {{ displayHint }}
    </p>
    ...
    <div v-if="parseError">
      <div class="flex items-center gap-2">
        <input
          class="min-w-0 flex-1 rounded-md border border-white/[0.1] bg-black/[0.25] px-2 py-1.5 text-xs text-white/50 opacity-70"
          :value="modelValue"
          readonly
        />
        <button
          class="shrink-0 rounded-md border border-white/[0.1] px-2 py-1.5 text-xs text-white/[0.7] transition-all hover:bg-white/[0.12] hover:text-white"
          @click="clear"
        >
    ...
    <div v-else class="flex flex-wrap items-center gap-2">
      <input
        class="w-24 min-w-0 rounded-md border border-white/[0.1] bg-black/[0.25] px-2 py-1.5 text-xs text-white transition-all focus:border-[var(--accent-color)] focus:outline-none"
        :list="uid"
      ...
      />
      <select
        class="shrink-0 rounded-md border border-white/[0.1] bg-black/[0.25] px-2 py-1.5 text-xs text-white transition-all focus:border-[var(--accent-color)] focus:outline-none"
        :value="draft.op"
        @change="onOp"
      >
      ...
      <input
        ...
        class="w-32 min-w-0 rounded-md border border-white/[0.1] bg-black/[0.25] px-2 py-1.5 text-xs text-white transition-all focus:border-[var(--accent-color)] focus:outline-none"
      ...
      <template v-else-if="draft.kind === 'random'">
        <input
          class="w-16 min-w-0 rounded-md border border-white/[0.1] bg-black/[0.25] px-2 py-1.5 text-xs text-white transition-all focus:border-[var(--accent-color)] focus:outline-none"
          type="number"
          :placeholder="t('scriptEditor.variable.min')"
          :value="String(draft.randomMin ?? '')"
          @change="onRandomMin"
        />
        <span class="shrink-0 text-xs text-white/40">{{ t("scriptEditor.variable.rangeTo") }}</span>
        <input
          class="w-16 min-w-0 rounded-md border border-white/[0.1] bg-black/[0.25] px-2 py-1.5 text-xs text-white transition-all focus:border-[var(--accent-color)] focus:outline-none"
          type="number"
    ...
    <datalist :id="uid">
      <option v-for="v in variables" :key="v" :value="v"></option>
    </datalist>
  </div>
</template>
