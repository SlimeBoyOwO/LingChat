<template>
  <div class="mb-4">
    <label
      class="text-brand inline-flex items-center gap-1.5 font-medium"
      :title="t('scriptEditor.fieldRow.yamlKey', { key: field.key })"
    >
      {{ fieldLabelOf(field, eventType) }}
      <span v-if="field.required" class="text-xs text-red-400">{{
        t("scriptEditor.configTab.requiredMark")
      }}</span>
      <span v-else class="ml-auto text-xs font-normal text-white/35">{{
        t("scriptEditor.fieldRow.optional")
      }}</span>
    </label>

    <!-- 遗留字段：只展示，不给编辑 -->
    <template v-if="field.kind === 'deprecated' || !field.enabled">
      <input class="glass-input opacity-45" :value="asText" disabled />
    </template>
    ...
    <!-- 开关。必填字段只有开/关两态；可选字段必须能表达「不设置」——
         ...
         作者点一下再点回来就悄悄改变了行为。 -->
    <div v-else-if="field.kind === 'bool' && field.required" class="flex items-center gap-2">
      <Toggle :checked="value === true" @change="(v: boolean) => emit('update', v)" />
    ...
    <!-- 固定候选 / 角色 / 情绪 / 章节 -->
    <select v-else-if="isSelectLike" class="glass-input" :value="asText" @change="onSelect">
      <!-- 非必填字段：不设置。character 例外：独立剧本无 MAIN 选项时，
           留空即引擎的 MAIN（当前主角），故也提供空选项并明确提示 -->
      <option v-if="!field.required || field.kind === 'character'" value="">
        {{
    ...
    <div v-else-if="field.kind === 'asset'">
      <div class="flex gap-2">
        <select class="glass-input" :value="asText" @change="onSelect">
          <option v-if="!field.required" value="">
            {{ t("scriptEditor.configTab.notSet") }}
          </option>
          <option v-for="name in assetOptions" :key="name" :value="name">
            {{ name }}
          </option>
        </select>
        <button
          class="shrink-0 rounded-lg border border-white/[0.1] bg-white/[0.06] px-[0.7rem] text-[0.78rem] whitespace-nowrap text-white/[0.7] transition-all hover:bg-white/[0.14] hover:text-white"
        ...
        <button
          class="shrink-0 rounded-lg border border-[rgba(167,139,250,0.3)] bg-[rgba(167,139,250,0.1)] px-[0.7rem] text-[0.78rem] whitespace-nowrap text-[#c4b5fd] transition-all hover:bg-[rgba(167,139,250,0.22)]"
      ...
      <p v-if="assetOptions.length === 0" class="mt-1 text-xs text-yellow-200">
        {{ t("scriptEditor.fieldRow.noAssetsHint") }}
      </p>
      <p v-else-if="globalOnly.length" class="mt-1 text-xs text-white/35">
    ...
    >
      ...
      <option v-for="opt in clothesOptions" :key="opt.value" :value="opt.value">
        {{ opt.label }}
      </option>
    </select>
    ...
    <p v-if="field.hint" class="mt-1 text-xs leading-relaxed" :class="hintClass">
