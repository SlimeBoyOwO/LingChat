...
<template>
  <MenuPage>
    ...
    <MenuItem :title="t('scriptEditor.characters.playerSection')">
      ...
      <div
        class="flex w-full items-center gap-3 rounded-[10px] border border-white/10 bg-white/6
          px-[13px] py-[11px]"
      >
        <div
    ...
    <MenuItem :title="t('scriptEditor.characters.menuTitle')">
      ...
      <p v-if="store.characters.length === 0" class="py-8 text-center text-[0.85rem] text-white/45">
      ...
      >
        ...
        >
          ...
          <span
            v-else
            class="flex h-full w-full items-center justify-center text-[0.56rem] text-white/35"
        ...
        <div class="flex min-w-0 flex-1 flex-col gap-0.5">
          ...
          <p v-if="!c.previewImage" class="mt-1 text-xs text-yellow-200">
            {{ t("scriptEditor.characters.noPortraitWarn") }}
          </p>
          <p v-else class="mt-1 text-xs text-white/40">
      ...
      <div class="mt-4 flex flex-wrap gap-2">
        <Button type="big" @click="emit('new-character')">
          {{ t("scriptEditor.characters.newCharacter") }}
        </Button>
        <Button type="big" @click="emit('import-character')">
