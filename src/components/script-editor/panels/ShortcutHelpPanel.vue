...
<template>
  ...
  <Teleport to="#app">
    ...
    >
      ...
      >
        <div
          class="max-h-[86dvh] w-[min(460px,92vw)] overflow-y-auto rounded-xl border border-white/12.5 bg-[rgba(12,20,30,0.86)] px-[18px] py-4 pb-[18px] shadow-[0_8px_32px_rgba(0,0,0,0.45),inset_0_1px_1px_rgba(255,255,255,0.06)] backdrop-blur-lg backdrop-saturate-[1.4]"
        >
          <div class="border-brand mb-4 flex items-center gap-2 border-b-2 pb-2">
            <h4 class="font-semibold text-white">{{ t("scriptEditor.shortcutHelp.title") }}</h4>
            <button
          ...
          <p class="mb-3 text-[0.72rem] leading-[1.7] text-white/40">
          ...
          <div
            ...
            :key="a"
            class="flex items-center gap-3 border-t border-white/[0.06] py-1.5 text-[0.78rem] leading-[1.8] text-white/70 [&:first-child]:border-t-0"
            :class="recording === a ? 'text-brand' : 'cursor-pointer hover:text-white'"
            @click="recording = a"
          >
            <kbd
              class="text-brand min-w-[148px] shrink-0 rounded-[5px] border border-white/[0.14] bg-white/5 px-2 py-0.5 font-mono text-[0.7rem]"
            >
              {{
                recording === a
            ...
            <span class="min-w-0 flex-1">{{ t(DESC_KEYS[a]) }}</span>
            <span
              v-if="recording === a && conflictName"
              class="shrink-0 text-[0.68rem] text-yellow-300"
            >
          ...
          <div class="mt-4 flex justify-end">
            <button
              class="inline-flex items-center gap-1 rounded-lg border border-white/10 bg-white/6 px-3 py-[0.3rem] text-[0.78rem] whitespace-nowrap text-white/70 transition-all duration-200 hover:enabled:bg-white/[0.12] hover:enabled:text-white"
