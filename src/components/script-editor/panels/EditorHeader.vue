...
<template>
  <div>
    ...
    <div class="flex w-full items-center justify-between px-5 py-2">
      <span class="text-brand ml-5 text-[0.95rem] font-bold tracking-[0.5px] whitespace-nowrap">{{
      ...
      >
        ...
        >
          <p class="hidden xl:block">{{ t(tab.label) }}</p>
          <span
            v-if="tab.key === 'validate' && store.report && store.report.errorCount > 0"
            class="ml-1 rounded-full bg-red-500 px-[5px] text-[0.6rem] text-white"
    ...
    <div class="flex items-center gap-2 px-8 pb-1 text-[0.8rem] text-white/55">
      <button v-if="store.detail" class="text-brand hover:underline" @click="store.closeScript()">
      ...
      <template v-if="store.detail">
        ...
        <b v-else class="font-semibold text-white">{{ store.detail.package.scriptName }}</b>
