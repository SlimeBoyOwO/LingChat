  ...
  <div class="flex flex-col items-center px-2 pt-[18px] pb-6">
    ...
    <template v-else>
      <div v-for="(row, ri) in rows" :key="row.key" class="flex w-full flex-col items-center">
        ...
        <div
          v-if="ri > 0"
          class="conn bg-brand/55 relative h-[34px] w-px after:absolute after:bottom-0
            after:left-[-4px] after:border-t-[8px] after:border-r-[4.5px] after:border-l-[4.5px]
            after:border-t-[rgba(121,217,255,0.6)] after:border-r-transparent
            after:border-l-transparent after:content-['']"
          :class="
            row.nodes.length > 1
              ? `fork before:absolute before:top-1/2 before:left-[-60px] before:h-px
                before:w-[120px] before:bg-[rgba(121,217,255,0.35)] before:content-[\'\']`
              : ''
          "
        >
          <span
            v-if="row.inboundLabels.length"
            class="absolute top-1/2 left-2.5 -translate-y-1/2 font-mono text-[9.5px]
        ...
        <div class="flex w-full flex-wrap justify-center gap-[18px]">
          <div
            ...
            :key="node.id"
            class="hover:border-brand group relative max-w-[460px] flex-[1_1_300px] cursor-pointer
              rounded-xl border border-white/12.5 bg-white/10 px-3.5 py-3
          ...
          >
            <span
              class="absolute -top-2 left-3 rounded border bg-[#16202c] px-1.5 py-px font-mono
                text-[9.5px]"
              :class="
                node.isIntro
            ...
            <div class="mt-2 flex min-h-4 items-center gap-1.5">
              <span
                v-if="node.errors"
                class="rounded border border-red-400/35 bg-red-400/15 px-[6px] py-px text-[0.64rem]
                  whitespace-nowrap text-red-300"
                >{{ t("scriptEditor.chapterFlow.errors", { count: node.errors }) }}</span
              >
              <span
                v-else-if="node.warns"
                class="rounded border border-amber-300/30 bg-amber-300/15 px-[6px] py-px
                  text-[0.64rem] whitespace-nowrap text-amber-300"
                >{{ t("scriptEditor.chapterFlow.warns", { count: node.warns }) }}</span
              >
              <span
                v-if="node.endType && node.endType !== 'linear'"
                class="rounded border border-purple-400/35 bg-purple-400/15 px-[6px] py-px
                  text-[0.64rem] whitespace-nowrap text-purple-300"
                >{{
              ...
              >
              <button
                class="ml-auto rounded px-[5px] py-px text-[11px] text-white/25 opacity-0
                  transition-all duration-150 group-hover:opacity-100 hover:bg-red-400/15
                  hover:text-red-300"
      ...
      ></div>
      <div
        class="rounded-full border border-dashed border-white/25 px-3.5 py-[5px] text-[0.72rem]
          whitespace-nowrap text-white/50"
      >
      ...
      <p
        class="[&_code]:text-brand mt-[26px] max-w-[560px] text-xs leading-[1.9] text-white/40
          [&_b]:text-white/65 [&_code]:font-mono"
      >
