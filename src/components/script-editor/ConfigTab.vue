...
<template>
  <MenuPage>
    <MenuItem :title="t('scriptEditor.config.menuTitle')">
      ...
      <div v-for="f in store.schema?.storyConfigFields ?? []" :key="f.key" class="mb-4">
        <label class="text-brand inline-flex items-center text-[0.9rem] font-medium">
          {{ storyFieldLabelOf(f)
          }}<span v-if="f.required" class="ml-0.5 text-[0.7rem] text-red-400">{{
      ...
      <div class="my-4 rounded-xl border border-white/10 bg-black/15 p-4">
        <label
          class="mb-2 inline-flex items-center gap-2 text-[0.8rem] whitespace-nowrap text-white/70"
        >
          <Toggle :checked="isAdventure" @change="toggleAdventure" />
        ...
        <template v-if="isAdventure">
          ...
          <div class="mb-4">
            ...
            <p
              class="mb-2 text-[0.72rem] leading-[1.7] text-white/40 [&_b]:font-semibold
                [&_b]:text-white/85"
              v-html="t('scriptEditor.configTab.unlockHint')"
            ></p>
            <div v-for="(cond, i) in conditions" :key="i" class="mb-2 rounded-lg bg-white/6 p-2.5">
              <div class="flex items-center gap-2">
                ...
                >
                  <option v-for="s in unlockSpecs" :key="s.typeKey" :value="s.typeKey">
                    {{ unlockTypeLabelOf(s) }}
                  </option>
                </select>
                <button
              ...
              >
                <span class="shrink-0 text-xs text-white/40">{{ unlockFieldLabelOf(f) }}</span>
                <input
                  v-if="f.kind === 'number'"
                  class="w-24 min-w-0 rounded-md border border-white/[0.1] bg-black/[0.25] px-2
                    py-1.5 text-xs text-white transition-all focus:border-[var(--accent-color)]
                    focus:outline-none"
                  type="number"
                ...
                <select
                  ...
                  "
                  class="min-w-0 flex-1 rounded-md border border-white/[0.1] bg-black/[0.25] px-2
                    py-1.5 text-xs text-white transition-all focus:border-[var(--accent-color)]
                    focus:outline-none"
                  :value="condField(cond, f.key)"
                ...
                >
                  ...
                  <option v-for="(title, id) in store.achievements" :key="id" :value="id">
                    {{ title }}（{{ id }}）
                  </option>
                </select>
                <!-- 冒险前置条件：下拉直选其他羁绊冒险 -->
                ...
                >
                  ...
                  <option v-for="o in adventureOptions" :key="o.value" :value="o.value">
                    {{ o.label }}
                  </option>
                </select>
                <input
            ...
            <button
              class="hover:border-brand hover:text-brand mt-1 rounded-lg border border-dashed
                border-white/15 px-3 py-1.5 text-xs text-white/45 transition-all"
              @click="addCondition"
            >
      ...
      <Button type="big" class="mt-4" @click="saveConfig">
