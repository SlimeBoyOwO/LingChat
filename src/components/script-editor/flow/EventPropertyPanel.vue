<template>
  <div class="flex flex-col gap-1">
    ...
    <template v-else>
      <!-- 事件类型 -->
      <div class="mb-4">
        <label class="text-brand inline-flex items-center font-medium">{{
          t("scriptEditor.eventProperty.eventType")
        }}</label>
        <select class="glass-input" :value="eventType" @change="onTypeChange">
          <optgroup v-for="(group, cat) in groupedSpecs" :key="cat" :label="categoryLabelOf(cat)">
            <option v-for="s in group" :key="s.typeKey" :value="s.typeKey">
      ...
      <div class="my-3 border-t border-white/10 pt-3">
        <p class="mb-2 text-xs tracking-wide text-white/35">
      ...
      <!-- 本事件上的诊断 -->
      <div v-if="eventDiagnostics.length" class="rounded-xl border border-white/10 bg-black/15 p-4">
