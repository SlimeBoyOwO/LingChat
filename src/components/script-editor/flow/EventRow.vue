<template>
  <div class="relative">
    <span
      class="absolute top-3 -left-[21px] h-[9px] w-[9px] rounded-full border-2 border-[#2b3a4a]"
      :style="{ background: spec?.color ?? '#64748b' }"
    ></span>
    <span
      v-if="conditionText"
      class="absolute -top-[7px] -left-[21px] rounded-[3px] border border-[rgba(251,191,36,0.32)]
        bg-[rgba(251,191,36,0.16)] px-[5px] font-mono text-[9px] whitespace-nowrap text-[#fcd34d]"
      >{{ t("scriptEditor.eventRow.conditionPrefix", { condition: conditionText }) }}</span
    >

    <div
      class="group flex items-start gap-2 rounded-lg border border-transparent px-[9px] py-1.5
        transition-all hover:bg-white/[0.07]"
      :class="{
    ...
    >
      <span
        class="shrink-0 rounded-[5px] border px-[7px] py-0.5 text-[0.7rem] leading-[1.5] font-medium
          whitespace-nowrap"
        :style="{
          color: spec?.color,
      ...
      >
        <template v-for="(part, i) in highlighted" :key="i">
          <span v-if="part.token" class="text-[var(--accent-color)] opacity-80">{{
            part.text
          }}</span>
      ...
      <span
        v-if="varBadge"
        class="max-w-[10rem] shrink-0 cursor-pointer truncate rounded border
          border-[rgba(34,211,238,0.35)] bg-[rgba(34,211,238,0.14)] px-[5px] py-px text-[0.62rem]
          leading-[1.6] whitespace-nowrap text-[#67e8f9] transition-all
      ...
      <span
        v-if="errorCount"
        class="shrink-0 rounded border border-[rgba(248,113,113,0.35)] bg-[rgba(248,113,113,0.15)]
          px-[5px] py-px text-[0.62rem] leading-[1.6] whitespace-nowrap text-[#fca5a5]"
        >{{ t("scriptEditor.chapterFlow.errors", { count: errorCount }) }}</span
      >
      <span
        v-else-if="warnCount"
        class="shrink-0 rounded border border-[rgba(251,191,36,0.3)] bg-[rgba(251,191,36,0.15)]
          px-[5px] py-px text-[0.62rem] leading-[1.6] whitespace-nowrap text-[#fcd34d]"
      ...
      <button
        class="shrink-0 rounded px-[3px] text-[11px] leading-[1.7] text-white/25 opacity-0
          transition-all group-hover:opacity-100 hover:bg-white/[0.1]
      ...
      <button
        class="shrink-0 rounded px-[3px] text-[11px] leading-[1.7] text-white/25 opacity-0
          transition-all group-hover:opacity-100 hover:bg-[rgba(248,113,113,0.15)]
          hover:text-[#fca5a5]"
