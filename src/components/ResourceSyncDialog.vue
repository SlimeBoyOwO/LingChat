<template>
  <Teleport to="body">
    <Transition name="modal">
      ...
      >
        <div
          class="flex max-h-[80dvh] w-[520px] flex-col overflow-hidden rounded-3xl bg-slate-800/90 shadow-2xl"
        >
          <!-- 头部 -->
          ...
          <!-- 内容区 -->
          <div class="scrollbar-hide flex-1 overflow-y-auto px-6 pb-2">
            <!-- 评审阶段：文件差异列表 -->
            <template v-if="phase === 'review' && syncInfo">
              <!-- 统计栏 -->
              <div
                class="mb-3 flex items-center justify-between rounded-xl bg-slate-700/50 px-4 py-2 text-sm text-slate-300"
              >
                ...
                <div class="flex gap-2">
                  <button class="text-xs text-blue-400 hover:text-blue-300" @click="selectAll">
                    {{ $t("ui.resourceSync.selectAll") }}
                  </button>
                  <button class="text-xs text-slate-400 hover:text-slate-300" @click="deselectAll">
                    {{ $t("ui.resourceSync.deselectAll") }}
                  </button>
                </div>
              </div>

              <!-- 文件树：按目录分组 -->
              <div class="max-h-[50dvh] space-y-1 overflow-y-auto">
                <template v-for="group in fileGroups" :key="group.dir">
                  <!-- 目录头 -->
                  <div
                    class="flex cursor-pointer items-center gap-2 rounded px-2 py-1 hover:bg-slate-700/40"
                  ...
                  >
                    ...
                    <span class="text-sm font-medium text-slate-300">{{ group.dir }}</span>
                  ...
                  <template v-if="group.expanded">
                    <div
                      ...
                      :key="file.path"
                      class="flex items-center gap-2 rounded py-1 pr-2 pl-8 hover:bg-slate-700/30"
                    >
                      <input
                        type="checkbox"
                      ...
                      />
                      <span class="flex-1 truncate text-sm text-slate-300" :title="file.path">
                      ...
                      >
                        {{
                          file.changeType === "add"
                            ? $t("ui.resourceSync.changeAdd")
                            : $t("ui.resourceSync.changeModify")
                        }}
                      </span>
            ...
            <template v-else-if="phase === 'syncing'">
              <div class="flex flex-col items-center gap-4 py-8">
                <RefreshCw :size="40" class="animate-spin text-blue-400" />
            ...
            <template v-else-if="phase === 'error'">
              <div class="flex flex-col items-center gap-4 py-8">
                ...
                <p class="text-center text-sm text-red-400">
                  {{ errorMessage || $t("ui.resourceSync.syncFailed") }}
                </p>
              </div>
            </template>
          </div>
          ...
          <div class="flex justify-end gap-3 border-t border-slate-700/50 px-6 py-4">
            ...
            <template v-else-if="phase === 'complete' || phase === 'error'">
              <button
                class="rounded-lg bg-slate-600 px-4 py-2 text-sm text-white hover:bg-slate-500"
                @click="$emit('close')"
              >
...
const { t } = useI18n();
import { Folder, Minus, Plus, RefreshCw, CheckCircle, XCircle } from "lucide-vue-next";
...
const fileGroups = computed<FileGroup[]>(() => {
  ...
  const allFiles = [...props.syncInfo.filesToAdd, ...props.syncInfo.filesToModify];
