    ...
    <Transition name="picker-fade">
      ...
      >
        ...
        <div
          ...
          class="relative mx-3 mb-6 w-full max-w-md overflow-hidden rounded-2xl border border-white/10 bg-neutral-900/95 shadow-2xl"
        >
          <div class="px-5 pt-5 pb-2 text-sm font-medium text-white/90">
            {{ $t("ui.imageSourcePicker.title") }}
          </div>

          <button
            type="button"
            class="flex w-full items-center gap-3 px-5 py-4 text-left text-white transition-colors hover:bg-white/10 active:bg-white/15"
            @click="onCamera"
          >
            <Camera :size="20" class="shrink-0 text-cyan-400" />
            <div class="flex-1">
          ...
          <button
            type="button"
            class="flex w-full items-center gap-3 border-t border-white/5 px-5 py-4 text-left text-white transition-colors hover:bg-white/10 active:bg-white/15"
            @click="onGallery"
          >
            <Image :size="20" class="shrink-0 text-cyan-400" />
            <div class="flex-1">
          ...
          <button
            type="button"
            class="w-full border-t border-white/10 px-5 py-3 text-sm text-white/70 transition-colors hover:bg-white/10 active:bg-white/15"
            @click="onCancel"
          >
...
const { isOpen, init, destroy, pickFromCamera, pickFromGallery, cancel } = useImageSourcePicker();
...
function onVisibilityChange() {
  if (typeof document !== "undefined" && document.visibilityState === "hidden" && isOpen.value) {
    cancel();
  }
}
