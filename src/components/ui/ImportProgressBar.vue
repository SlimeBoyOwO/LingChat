<template>
  <Teleport to="body">
    ...
    <div
      ...
      class="notice fixed top-8 right-8 z-[10000] flex max-w-[420px] min-w-[320px] items-start gap-3 rounded-xl p-4 backdrop-blur-[20px]"
    >
    ...
    <Transition name="slide-up">
      <div
        ...
        class="bar fixed right-8 bottom-[calc(32px+var(--safe-area-inset-bottom))] z-[9999] flex max-w-[440px] min-w-[340px] items-center gap-4 overflow-hidden rounded-xl p-4 backdrop-blur-[20px]"
      >
        <div
          v-if="state.phase !== 'cancelled'"
          class="glow absolute top-1/2 left-1/2 -z-10 h-[150%] w-[150%] -translate-x-1/2 -translate-y-1/2 blur-[20px]"
          :style="glowStyle"
        ></div>
        ...
        <button
          v-if="state.phase === 'running'"
          class="btn-cancel shrink-0 cursor-pointer self-start rounded-md px-3 py-1.5 text-xs font-medium text-white/80"
          @click="onCancel"
        >
...
const label = computed(() => {
  ...
  if (p === "done")
    return isImport ? t("ui.archiveProgress.importSuccess") : t("ui.archiveProgress.exportSuccess");
  if (p === "error")
