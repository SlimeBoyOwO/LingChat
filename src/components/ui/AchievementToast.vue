...
<script setup lang="ts">
  ...
  const typeLabel = computed(() => {
    return store.current?.type === "rare"
      ? t("ui.achievementToast.rare")
      : t("ui.achievementToast.unlocked");
  });
...
<style scoped>
  ...
  .achievement-toast {
    ...
    @apply max-w-[400px] min-w-[320px] p-4;
  ...
  .achievement-green .achievement-label {
    @apply text-xs font-bold tracking-wider text-green-400;
  }
  ...
  .achievement-gold .achievement-label {
    @apply text-xs font-bold tracking-widest text-amber-300 uppercase;
  ...
  .achievement-content {
    @apply flex flex-1 flex-col justify-center gap-0.5;
  }

  .achievement-title {
    @apply text-sm leading-tight font-bold text-white;
  }

  .achievement-description {
    @apply text-xs leading-tight text-gray-300;
  }

  /* 进度条容器 */
  .progress-bar-container {
    @apply absolute bottom-0 left-0 h-[2px] w-full bg-gray-800/50;
  }
