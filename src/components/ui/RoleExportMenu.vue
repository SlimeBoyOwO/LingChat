<template>
  <div class="relative">
    <button class="export-trigger" :title="$t('ui.roleExport.title')" @click.stop="toggle">
      <Upload :size="24" />
    </button>
    ...
    <Transition name="menu-pop">
      <div v-if="open" class="export-menu" @click.stop>
        <div class="menu-header">
          {{
            roleName
              ? $t("ui.roleExport.headerWithName", { name: roleName })
              : $t("ui.roleExport.title")
          }}
        </div>
        <button class="menu-item" :disabled="busy" @click="choose('zip')">
          ...
          <span class="hint">{{
            busy && format === "zip" ? $t("ui.roleExport.processing") : $t("ui.roleExport.zipHint")
          }}</span>
        </button>
        <button class="menu-item" :disabled="busy" @click="choose('7z')">
          ...
          <span class="hint">{{
            busy && format === "7z" ? $t("ui.roleExport.processing") : $t("ui.roleExport.p7zHint")
          }}</span>
        </button>
      </div>
    </Transition>
...
const { doExport } = useRoleImportExport();
...
.export-trigger {
  ...
  @apply bg-black/5 text-white/60 hover:bg-white/10 hover:text-white;
...
.menu-header {
  @apply px-3 py-1.5 text-xs font-bold tracking-wider text-white/50;
  border-bottom: 1px solid rgba(255, 255, 255, 0.06);
  margin-bottom: 4px;
}

.menu-item {
  @apply flex w-full items-center gap-2.5 rounded-lg px-3 py-2;
  @apply cursor-pointer text-left transition-colors;
