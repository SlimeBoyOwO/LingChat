<template>
  ...
  >
    <div
      ...
      :key="viewKey"
      class="pointer-events-none mr-2 hidden h-10 max-w-[min(36vw,28rem)] min-w-0 items-center
        xl:flex"
      role="status"
      aria-live="polite"
      :title="statusText"
    >
      <div
        class="flex min-w-0 items-center gap-1.5 text-xs font-medium tracking-wide drop-shadow-sm"
        :class="statusClass"
      >
        <LoaderCircle
        ...
        />
        <CheckCircle2 v-else-if="activity?.status === 'success'" :size="14" class="shrink-0" />
        <CircleAlert v-else :size="14" class="shrink-0" />
...
<script setup lang="ts">
  ...
  const target = computed(() => {
    ...
    try {
      ...
      const keys = ["search_files", "grep_files", "glob", "grep", "web_search"].includes(
        current.tool
      )
        ? ["query", "pattern", "path"]
  ...
  function runningKey(current: NonNullable<typeof activity.value>): string {
    ...
    if (["web_search", "search_files", "grep_files", "glob", "grep"].includes(tool))
      return "searching";
