<template>
  <transition appear :css="false" @before-enter="beforeEnter" @enter="enter" @leave="leave">
    ...
    <div
      v-if="isVisible"
      class="pointer-events-none fixed inset-0 z-1000 mt-[-20dvh] flex flex-col items-center justify-center"
    >
      <!-- 柔和的雾化背景层 -->
      <div
        class="relative flex w-full flex-col items-center justify-center border-y border-white/10 bg-slate-900/40 py-12 shadow-[0_0_25px_rgba(0,0,0,0.8)] backdrop-blur-2xl"
      >
        ...
        <div class="pointer-events-none absolute inset-0 -z-10 mask-center opacity-60">
          <div
            class="animate-float-slow absolute top-4 left-[30%] h-1.5 w-1.5 rounded-full bg-white/60"
          ></div>
          <div
            class="bg-brand/60 animate-float absolute right-[35%] bottom-6 h-1 w-1 rounded-full"
          ></div>
          <div
            class="animate-float-reverse absolute top-8 right-[30%] h-2 w-2 rounded-full bg-purple-400/50 blur-[1px]"
          ></div>
          <div
            class="animate-float-slow absolute bottom-4 left-[40%] h-1 w-1 rounded-full bg-cyan-300/40"
          ></div>
        </div>

        <!-- 柔和微光扫射 -->
        <div
          class="animate-shine-fast absolute -inset-full top-0 -z-10 block h-full w-[200%] -skew-x-12 transform bg-linear-to-r from-transparent via-white/5 to-transparent mask-center"
        ></div>
        ...
        <div class="relative z-10 flex flex-col items-center">
          ...
          <div class="mb-3 flex items-center gap-4 opacity-90">
            ...
            <span
              class="text-brand text-shadow-glow translate-x-[0.25em] text-sm font-light tracking-[0.5em] uppercase"
            >
              Story Clear
            </span>
          ...
          <!-- 剧本名称主标题 -->
          <h2
            class="mb-2 text-4xl font-bold tracking-widest text-white drop-shadow-[0_4px_16px_rgba(0,0,0,0.8)] md:text-5xl"
          >
            {{ completedScriptName }}
          </h2>
