<template>
  <transition @before-enter="beforeEnter" @enter="enter" @before-leave="beforeLeave" @leave="leave">
    <div
      v-if="isVisible"
      class="pointer-events-auto fixed bottom-[calc(24px+var(--safe-area-inset-bottom))] left-0 z-1000 drop-shadow-[0_8px_12px_rgba(0,0,0,0.25)]"
    >
      <div
        class="relative flex items-center gap-1 overflow-hidden rounded-tr-xl bg-slate-900/40 py-2 pr-5 pl-4 backdrop-blur-md"
      >
        <!-- 左侧装饰条 -->
        <div
        ...
        <div class="flex items-center gap-0.5">
          <button
            class="rounded-lg p-1.5 text-gray-400 transition-colors duration-200 hover:bg-white/10 hover:text-white"
