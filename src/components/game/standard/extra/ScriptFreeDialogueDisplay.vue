<template>
  ...
  <transition appear :css="false" @before-enter="beforeEnter" @enter="enter" @leave="leave">
    <div
      v-if="showAnimation"
      class="pointer-events-none fixed top-[20dvh] right-0 left-0 z-1000 flex justify-center"
    >
      <!-- 中间实体、两端渐变淡出透明的遮罩背景层 -->
      <div
      ...
      >
        <div class="flex items-center gap-4 opacity-90">
          <!-- 左侧渐隐线 -->
          <div class="to-brand/80 h-px w-10 bg-linear-to-r from-transparent"></div>
          ...
          <!-- 右侧渐隐线 -->
          <div class="to-brand/80 h-px w-10 bg-linear-to-l from-transparent"></div>
        </div>
      </div>
    </div>
  </transition>
  ...
  <transition name="slide-tag">
    <div v-if="shouldShowInfo" class="pointer-events-none fixed top-28 left-0 z-40">
      ...
      >
        <!-- 紧贴左侧边缘的高亮指示条 -->
        <div class="bg-brand absolute top-0 bottom-0 left-0 w-1"></div>
        ...
        <div class="flex items-center gap-4">
          ...
          <div v-if="freeDialogueInfo.endLine" class="flex items-center gap-1.5">
            ...
            <span class="text-brand text-[13px] font-medium">
