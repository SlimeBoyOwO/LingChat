<template>
  <!-- 背景图 + 背景光照滤镜 -->
  <div v-if="backgroundSrc" class="absolute inset-0" :style="bgLightingFilter">
    <ImageAcrossFade
      ref="imageFadeRef"
  ...
  <div class="pointer-events-none absolute inset-0" style="isolation: isolate">
    ...
    <Fireworks
      ...
      :style="`z-index:${BACKGROUND_ZINDEX}`"
    />
    <!-- 星辉：原桌宠专用粒子，主界面按全屏尺度放大数量（桌宠圆头像用 60/0.2） -->
    <BAParticles
...
// 这里只取换算好的背景侧样式，不再各自读 currentScene.lighting。
...
const effectiveVolume = (trackVolume: number) =>
  (trackVolume / 100) * (uiStore.ambientVolume / 100);
