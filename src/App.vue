...
<script setup lang="ts">
  ...
  // Vue DevTools 仅在开发模式注入，按设置控制所有窗口中的悬浮面板。
  ...
  // DevTools 容器可能晚于 App 挂载，出现时再应用一次显示设置。
  let devtoolsObserver: MutationObserver | null = null;
  if (import.meta.env.DEV) {
    devtoolsObserver = new MutationObserver(applyVueDevToolsVisibility);
