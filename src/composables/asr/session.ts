  ...
  try {
    ...
    if (isStreamEnabled() && !isLlamaStream()) {
      await asrStartStreaming({
        providerId: runtime.asrStore?.settings.active_provider ?? "openai-whisper",
        languageHint: null,
      });
    }
    ...
    runtime.processor.onaudioprocess = (e) => {
      ...
      if (source === "auto") {
        ...
        // VAD 端点检测只需要最近的音频）
        if (runtime.vadPending.length > 8192) {
          runtime.vadPending.splice(0, runtime.vadPending.length - 8192);
        }
        feedVad();
      }
      if (isStreamEnabled() && !isLlamaStream()) {
        ...
        if (runtime.streamPending.length > 8192) {
          runtime.streamPending.splice(0, runtime.streamPending.length - 8192);
        }
        feedStream();
      }
...
async function doRecognize(source: AsrSource, captured: number[]) {
  try {
    ...
    const providerId = runtime.asrStore?.settings.active_provider ?? "openai-whisper";
 ...
 */
export function handle(text: string, source: AsrSource) {
