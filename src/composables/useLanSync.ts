...
async function setupEventListeners() {
  ...
  unlistenProgress = await listen<SyncProgressEvent>("lan-sync-progress", (event) => {
  ...
  unlistenComplete = await listen<SyncResult>("lan-sync-complete", (event) => {
    lastResult.value = event.payload;
    phase.value = event.payload.success ? "complete" : "error";
    errorMessage.value = event.payload.message;
  });

  unlistenPeers = await listen<PeerInfo[]>("lan-sync-peers-updated", (event) => {
