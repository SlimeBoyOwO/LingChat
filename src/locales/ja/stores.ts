export default {
  ...
  llmErrors: {
    invalid_api_key:
      "API キーが正しくありません！API キーの入力（余分なスペース含む）、期限切れ、残高不足を確認してください。",
    forbidden:
      "アクセスが拒否されました（403）：このエンドポイントに対する権限を確認してください。",
    not_found:
      "エンドポイントまたはモデルが見つかりません（404）：Base URL のパスとモデル名を確認してください。",
    rate_limited:
      "リクエストが多すぎます（429）：時間をおいて再試行するか、回数・残高制限を確認してください。",
    server_error:
      "プロバイダのサーバーエラー（5xx）：時間をおいて再試行するか、プロバイダの稼働状況を確認してください。",
    timeout:
      "リクエストがタイムアウトしました：ネットワーク状況、プロバイダの負荷、タイムアウト設定を確認してください。",
    network_error:
      "ネットワーク接続に失敗しました：ネットワーク、Base URL の到達可能性、プロキシを確認してください。",
    empty_response:
      "モデルが空の応答を返しました：別のモデルを試すか、パラメータを確認してください。",
