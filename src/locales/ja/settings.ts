export default {
  ...
  advanceOther: {
    ...
    semanticMemoryStatus: {
      ...
      refresh: "状態を更新",
      needEmbedding:
        "注意: セマンティックメモリーは埋め込みモデルに依存します。先に「メモリー埋め込み」でモデルが準備済みか確認してください。",
    },
    ...
    fields: {
      ...
      embedding: {
        ...
        model_dir:
          "埋め込みモデルのディレクトリ（空欄時はデフォルトの data/third_party/embedding/、パッケージ版はリソース内の内蔵モデルにフォールバック、相対パスはデータディレクトリ基準で解決）",
        backend: "バックエンド: auto / onnx（Rust 側では同一、st は torch が必要で非対応）",
      },
      semantic_memory: {
        enabled:
          "独立セマンティックメモリーを有効にするか（ベクトルストア + AI 自動書き込み + 自動リコール）",
