export default {
  ...
  advanceOther: {
    ...
    fields: {
      ...
      startup: {
        pet_mode:
          "ペットモードで起動：オンにすると、exe を手動で起動した際に直接ペットモードで開きます（「ペットで自動起動」のペットスイッチとは独立、Windows のみ）",
        auto_play:
          "自動会話を再生：オンにすると、会話シーンでデフォルトで自動再生スイッチがオンになります（全体で有効）",
        auto_start_tts:
          "起動時に API サービスを自動起動：オンにすると、ペットモードでも通常起動でも起動時に外部 TTS サービスを起動・更新します（下の起動スクリプトが必要）",
