export default {
  ...
  advanceOther: {
    ...
    embeddingStatus: {
      title: 'メモリー埋め込みの実行状態',
      desc: 'メモリー埋め込みは Python サブプロセスで動作します。未設定・モデル欠落・サブプロセス起動失敗の場合は、セマンティック検索と重複排除が自動的に無効化され、通常の会話には影響しません。',
      enabled: '有効状態',
      ready: 'サービス準備完了',
      configured: '設定完了',
      dim: 'ベクトル次元',
    ...
    semanticMemoryStatus: {
      title: 'セマンティックメモリーの実行状態',
      desc: '通常の記憶庫（MemoryBank）やノートとは独立した専用のセマンティックメモリーです。AI は semantic_mem_* ツールで SQLite ベクトルストアに書き込み、各会話で関連フラグメントを自動的に文脈へ注入します。モデルエンコードを行うには「メモリー埋め込み」の有効化が必要です。',
      enabled: '有効状態',
      opened: 'ベクトルストア起動済み',
      embeddingReady: '埋め込みエンジン準備完了',
      count: 'メモリー件数',
      dbPath: 'ベクトルストアファイル',
      loading: '読み込み中…',
      refresh: '状態を更新',
      needEmbedding: '注意: セマンティックメモリーは埋め込みモデルに依存します。先に「メモリー埋め込み」でモデルが準備済みか確認してください。',
    },
    semanticMemoryManage: {
      title: 'セマンティックメモリー管理',
      desc: '各キャラクターがベクトルストアに保存したセマンティックメモリーを手動で表示・管理します（AI が semantic_mem_* ツールで書き込んだデータと同じものです）。照会・追加・編集・削除が可能です。',
      role: 'キャラクター',
      currentRole: '現在の会話',
      noneRole: '利用可能なキャラクターがいません',
      addTitle: 'メモリーを追加',
      editTitle: 'メモリーを編集',
      contentPlaceholder: '覚えておきたい内容を入力…',
      tagsPlaceholder: 'タグ（カンマ区切り、任意）。例: 設定, 背景, 重要事項',
      add: '追加',
      save: '変更を保存',
      cancelEdit: 'キャンセル',
      loading: '読み込み中…',
      empty: 'このキャラクターのセマンティックメモリーはまだありません',
      emptyTags: 'タグなし',
      edit: '編集',
      delete: '削除',
      deleteConfirm: 'このセマンティックメモリーを削除しますか？',
      added: '追加しました',
      updated: '変更を保存しました',
      loadFailed: '読み込み失敗: {error}',
    },
    ...
    fields: {
      ...
      embedding: {
        enabled: 'メモリー埋め込みを有効にするか（セマンティック検索 + 手動ノートの重複排除）',
        model_dir: '埋め込みモデルのディレクトリ（空欄時はデフォルトの data/third_party/embedding/、パッケージ版はリソース内の内蔵モデルにフォールバック、相対パスはデータディレクトリ基準で解決）',
        backend: 'バックエンド: auto / onnx（Rust 側では同一、st は torch が必要で非対応）',
      },
      semantic_memory: {
        enabled: '独立セマンティックメモリーを有効にするか（ベクトルストア + AI 自動書き込み + 自動リコール）',
        top_k: '各会話で文脈に自動注入する最大リコール件数（1–20、デフォルト 4）',
      },
      log: {
  ...
  tts: {
    ...
    cosyvoice: {
      title: '音声クローン TTS',
      subtitle: 'Alibaba Cloud Bailian CosyVoice を利用し、音声サンプルからオリジナル音声をクローン',
      ...
      importHint: '長さは 10〜20 秒を推奨、対応形式: wav / mp3、サンプルレート 16kHz 以上、選択した参照音声言語は実際の音声言語と一致させる必要があります、単一の話者が明瞭に発話、BGM やノイズなし',
