export default {
  ...
  toolCalls: {
    ...
    accessModeHint: '只读工具（Read / Glob / Grep / ReadMediaFile）始终直接运行；这里控制文件修改和命令执行是否需要确认。',
    ...
    accessModes: {
      ...
      full_access: { label: '完全访问', description: '不再询问，可读写电脑上的任意路径并执行命令。' },
    },
    ...
    adminModeElevated: 'LingChat 已以管理员权限运行；uac=true 命令会直接复用当前权限，不再重复弹出 UAC。',
    ...
    adminModeConfirmMessage: 'Windows 将显示一次正常的 UAC 确认。重启后，完全访问模式下执行的命令会继承管理员权限。是否继续？',
    ...
    mediaVideoCompatibility: '⚠ 视频通过 OpenAI 兼容的 video_url 发送；实际支持情况取决于视觉模型供应商。',
    ...
    kimiHint: 'API Key 可留空：当前对话模型为官方 Kimi Code 时，会安全复用它的凭据；其他情况请单独填写',
    ...
    dsModel: 'DeepSeek 模型',
    dsHint: '通过 DeepSeek Responses API 的服务端 web_search 联网；需要单独填写 DeepSeek API Key，结果由服务端生成综合回答',
    baseUrl: '搜索服务地址',
