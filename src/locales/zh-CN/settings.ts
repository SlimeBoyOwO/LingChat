export default {
  ...
  advanceOther: {
    ...
    embeddingStatus: {
      title: "记忆嵌入运行状态",
      desc: "记忆嵌入由 Python 子进程承载。未配置、模型缺失或子进程启动失败时，语义检索与去重会自动停用，不影响正常对话。",
      enabled: "启用状态",
      ready: "服务就绪",
      configured: "配置齐备",
      dim: "向量维度",
      ...
      loading: "加载中…",
      refresh: "刷新状态",
      organizeConversation: "一键整理当前对话",
      organizeResult:
        "整理完成：新增 {added} 条，跳过重复 {duplicates} 条，当前存档已归档 {stored} 条。",
    ...
    semanticMemoryStatus: {
      title: "语义记忆运行状态",
      desc: "独立于普通记忆库（MemoryBank）与笔记的专属语义记忆，由 AI 通过 semantic_mem_* 工具写入 SQLite 向量库，每轮对话自动召回相关片段注入上下文。需要先启用「记忆嵌入」提供模型编码。",
      enabled: "启用状态",
      opened: "向量库已打开",
      embeddingReady: "嵌入引擎就绪",
      count: "记忆条数",
      dbPath: "向量库文件",
      loading: "加载中…",
      refresh: "刷新状态",
      needEmbedding: "提示：语义记忆依赖嵌入模型，请先在「记忆嵌入」中确认模型已就绪。",
    },
    semanticMemoryManage: {
      title: "语义记忆管理",
      desc: "手动查看和管理角色保存在语义记忆库中的内容（与 AI 通过 semantic_mem_* 工具写入的是同一份数据），支持查询、新增、编辑与删除。",
      role: "角色",
      currentRole: "当前对话",
      noneRole: "暂无可用角色",
      addTitle: "新增记忆",
      ...
      tagsPlaceholder: "标签（用逗号分隔，可选），如：设定, 背景, 重要事项",
      add: "添加",
      save: "保存修改",
      cancelEdit: "取消编辑",
      loading: "加载中…",
    ...
    fields: {
      ...
      embedding: {
        enabled: "是否启用记忆嵌入（语义检索 + 手动笔记去重）",
        model_dir:
          "嵌入模型目录（留空时默认 data/third_party/embedding/，打包版自动回退资源目录内置模型，相对路径按数据目录解析）",
        backend: "后端：auto / onnx（Rust 侧一致，st 需 torch 不再支持）",
      },
      semantic_memory: {
        enabled: "是否启用独立语义记忆（向量库 + AI 自动写入 + 自动召回）",
        top_k: "每轮对话自动注入上下文的最多召回条数（1–20，默认 4）",
      },
      log: {
  ...
  characterInfo: {
    ...
    fields: {
      ...
      sherpaOnnx: "Sherpa-ONNX 本地语音",
      sherpaOnnxModelName: "Sherpa-ONNX 模型",
      sherpaOnnxModelType: "模型类型",
      sherpaOnnxLanguage: "语言",
      sherpaOnnxVoice: "音色",
      sherpaOnnxUseGpu: "GPU 推理",
      sherpaOnnxSpeed: "语速",
      sherpaOnnxParameters: "参数",
      sherpaOnnxManageModels: "管理模型",
      sherpaOnnxVoicesMale: "男声",
      sherpaOnnxVoicesFemale: "女声",
      sherpaOnnxVoicesChild: "童声",
      sherpaOnnxVoicesElderly: "老年声",
      sherpaOnnxModelDir: "模型存放目录",
      sherpaOnnxStoragePermHint:
        "模型存放于应用数据目录（data/sherpa_onnx_models），无需额外存储权限",
      sherpaOnnxGrantPermission: "立即授权",
      basicSettings: "基础语音设置",
      emotion: "情感",
      emotionsNormal: "普通",
      emotionsHappy: "高兴",
      emotionsSad: "难过",
      emotionsAngry: "生气",
      emotionsSurprised: "惊讶",
      noSherpaModel: "未安装 Sherpa-ONNX 模型（请先在 TTS 设置中下载）",
    },
    placeholders: {
  ...
  sherpa: {
    title: "Sherpa-ONNX 本地语音",
    models: "模型",
    noModels: "暂无可用模型，请先下载",
    manageModels: "打开模型文件夹",
    modelConfig: "模型配置",
    modelType: "模型类型",
    language: "语言",
    voice: "音色",
    ...
    speed: "语速",
    zeroShot: "零样本声音克隆",
    zeroShotDesc: "部分模型（如 kokoro/kitten 等）支持参考音频 + 参考文本的零样本克隆。",
    refLoaded: "参考音频已加载",
    refNone: "未选择参考音频",
    refChange: "更换音频",
    refPick: "选择音频",
    refClear: "清除",
    refText: "参考文本",
    refTextPlaceholder: "与参考音频对应的文本……",
    test: "试听",
    testPlaceholder: "输入要试听的文字……",
    synthesizing: "合成中…",
    listen: "试听",
    noModelHint: "请先选择一个已安装的模型",
  },
