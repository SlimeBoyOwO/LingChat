...
export default {
  ...
  advanceOther: {
    ...
    semanticMemoryStatus: {
      ...
      loading: "Loading…",
      refresh: "Refresh Status",
      needEmbedding: "Note: semantic memory depends on the embedding model. Please confirm it is ready under Memory Embedding first.",
    },
    ...
    fields: {
      ...
      embedding: {
        enabled: "Whether to enable memory embedding (semantic retrieval + deduplication of manual notes)",
        model_dir: "Embedding model directory (defaults to data/third_party/embedding/ when blank; packaged builds fall back to the bundled model in resources; relative paths resolve against the data directory)",
        backend: "Backend: auto / onnx (identical in Rust; st requires torch and is no longer supported)",
      },
      semantic_memory: {
        enabled: "Whether to enable the independent semantic memory (vector store + AI auto-write + auto recall)",
  ...
  tts: {
    ...
    cosyvoice: {
      ...
      subtitle: 'Powered by Alibaba Cloud Bailian CosyVoice — clone a custom voice from audio samples',
      ...
      importHint: 'Duration: 10–20 s recommended; wav / mp3; sample rate ≥ 16 kHz; the selected reference audio language must match the actual audio language; one clear speaker, no background music or noise',
      ...
      notices: {
        ...
        voiceSubmitted: 'Voice submitted for review: {name} ({voiceId}). It will be usable once approved',
        registerFailed: 'Voice registration failed: {error}',
        deleteConfirm: 'Delete voice "{name}"? Both the cloud voice and the local record will be removed',
