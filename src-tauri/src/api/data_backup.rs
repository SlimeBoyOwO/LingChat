...
pub async fn export_data_backup(
    ...
    if selections.settings {
        ...
        let json =
            serde_json::to_string_pretty(&map).map_err(|e| format!("serialize settings: {e}"))?;
...
pub async fn import_data_backup(
    ...
    if selections.frontend_preferences && manifest.selections.frontend_preferences {
        if let Ok(prefs) =
            tokio::fs::read_to_string(temp_dir.join("frontend_preferences.json")).await
        {
...
fn create_zip_from_dir(dir: &Path, zip_path: &Path) -> Result<(), String> {
    ...
    for path in &files {
        ...
        std::io::copy(&mut f, &mut zip).map_err(|e| format!("写入 zip {}: {e}", path.display()))?;
    }
...
fn extract_zip_to_dir_sync(zip_path: &str, dest_dir: &Path) -> Result<(), String> {
    ...
    let mut archive = zip::read::ZipArchive::new(file).map_err(|e| format!("读取 zip: {e}"))?;
    ...
    for i in 0..archive.len() {
        ...
        // 安全检查 1：拒绝路径遍历（按路径组件判断，避免误伤含连续点的正常文件名）
        ...
        // 先创建父目录：导出的 zip 只含文件条目（无目录条目），首层文件解压时父目录
        // 尚不存在。若不先创建，下方 canonicalize 失败后只能回退到未规范化路径——
        // Windows 上 canonicalize 返回 \\?\ 前缀的 verbatim 路径，与回退路径前缀不一致，
        // starts_with 必然失配，会把正常条目误判为路径遍历（Android/Linux 无此前缀不受影响）。
        if let Some(parent) = dest_path.parent() {
            std::fs::create_dir_all(parent).map_err(|e| format!("创建父目录 {name}: {e}"))?;
        }

        // 安全检查 3：规范化后校验仍在目标目录内（父目录已存在，canonicalize 必定成功）
        let canonical_dest = if let Some(parent) = dest_path.parent() {
            let canonical_parent = parent
                .canonicalize()
                .map_err(|e| format!("规范化父目录 {name} 失败: {e}"))?;
        ...
        if entry.is_dir() {
            std::fs::create_dir_all(&dest_path).map_err(|e| format!("创建目录 {name}: {e}"))?;
        } else {
            let mut out = File::create(&dest_path).map_err(|e| format!("创建文件 {name}: {e}"))?;
...
async fn copy_to_destination(app: &AppHandle, src: &Path, dest_path: &str) -> Result<(), String> {
    if dest_path.starts_with("content://") {
        ...
        tracing::info!("[DataBackup] SAF 复制: {} -> {}", src.display(), dest_path);
...
mod tests {
    ...
    /// 构造与 `create_zip_from_dir` 产出一致的测试 zip：只有文件条目（无目录条目）。
    ...
    /// 回归测试：Windows 下 canonicalize 返回 \\?\ verbatim 前缀路径，
    /// 若解压时父目录尚未创建（导出的 zip 无目录条目），旧的未规范化回退路径
    /// 会因前缀不一致被误判为路径遍历。中文文件名 + 深层嵌套 + 首条目即文件。
    #[test]
    fn extract_nested_unicode_files_without_dir_entries() {
        let base = std::env::temp_dir().join(format!("data_backup_test_{}", std::process::id()));
        ...
        write_test_zip(
            ...
            &[
                ...
                (
                    "resources/characters/DeepSeek/avatar/伤心.webp",
                    b"img-bytes",
                ),
                ("resources/characters/DeepSeek/avatar/开心.webp", b"img-2"),
            ],
        );

        let result = extract_zip_to_dir_sync(zip_path.to_str().unwrap(), &dest);
        assert!(result.is_ok(), "解压不应失败: {result:?}");
        let extracted = dest.join("resources/characters/DeepSeek/avatar/伤心.webp");
        assert_eq!(std::fs::read(&extracted).unwrap(), b"img-bytes");
    ...
    fn extract_rejects_traversal_entry() {
        let base = std::env::temp_dir().join(format!("data_backup_trav_{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&base);
        std::fs::create_dir_all(&base).unwrap();

        let zip_path = base.join("evil.zip");
        let dest = base.join("out");
        write_test_zip(&zip_path, &[("../evil.txt", b"pwned".as_slice())]);

        let result = extract_zip_to_dir_sync(zip_path.to_str().unwrap(), &dest);
        assert!(result.is_err());
        assert!(result.unwrap_err().contains("路径遍历攻击"));
