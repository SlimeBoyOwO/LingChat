//! 身份卡的磁盘存储：`<data_dir>/game_data/my_identities/`。
//!
//! 不复用 `characters/`：启动时会扫描该目录并把文件夹登记成角色，身份卡放进去
//! 会被误登记成第二个角色。独立目录同时保证旧版本完全不认识这些数据。
...
impl IdentityStore {
    ...
    /// 读取「当前身份」；从未设置过时按老规则合成一张并立即落盘
    /// （不落盘的话，老用户切换 AI 角色时「我」的名字仍会跟着角色卡变）。
    /// 落盘失败时返回内存里的合成卡（`synthetic = true`），只是不持久。
    ...
    /// 新建或更新一张身份卡。
    /// - `id` 为空 → 新建（生成不可变 id，按名字分配文件夹）；
    /// - `id` 已存在 → 沿用原文件夹（改名不搬家，关系与存档引用不断链）；
    /// - `id` 非空但找不到 → 按该 id 新建。
