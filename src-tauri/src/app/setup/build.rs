...
pub(super) fn build_service_graph(
    ...
    let embedding = if app_config.embedding.enabled {
        ...
        let cfg = app_config
            .embedding
            .to_service_config(&data_dir, resource_dir.as_deref());
        if cfg.enabled() {
            tracing::info!(
            ...
            Some(Arc::new(
                crate::ai_service::embedding::EmbeddingManager::new(cfg),
            ))
        } else {
            tracing::warn!(
    ...
    // 嵌入引擎未就绪时仍可打开库做列表/删除，但编码/检索不可用。
    let semantic_memory = if app_config.semantic_memory.enabled {
        let db_path = data_dir.join("game_data").join("semantic_memory.db");
        let top_k = app_config.semantic_memory.top_k as usize;
        match rt.block_on(crate::ai_service::semantic_memory::SemanticMemory::open(
            &db_path,
            embedding.clone(),
            top_k,
        )) {
            Ok(sm) => {
                tracing::info!("[semantic_memory] 已启用，向量库: {}", db_path.display());
                Some(Arc::new(sm))
            },
            Err(e) => {
