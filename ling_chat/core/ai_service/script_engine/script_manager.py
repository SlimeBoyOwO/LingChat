...
class ScriptManager:
    ...
    def _init_all_scripts(self):
        ...
        subdirs_to_scan = []
        # 支持分类子目录：character/ 和 standalone/
    ...
    async def _run_script(self, script: ScriptStatus):
        ...
        while next_chapter_name != "end":
            try:
                ...
                current_chapter_obj: Chapter = self._get_chapter(chapter_path, script)
        ...
        if script.running_client_id:
            await message_broker.publish(script.running_client_id, event_response.model_dump())
