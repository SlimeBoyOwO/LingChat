...
- 右手每次起音的最高音作为主旋律，共 574 个；先上移一个八度，再将所有声部统一降低两个半音，主旋律最终相对参考谱上移 **10 个半音**，保留高音旋律与伴奏之间的层次。
- 右手其余 197 个音符作为内声部和声，先下移一个八度，再跟随全曲降低两个半音，最终相对参考谱下移 14 个半音；左手的 546 个低音音符下移两个半音。三个声部采用相同的调性偏移，保持原谱的和声关系与节奏。
- 旋律、和声、低音分别设置音色、包络和音量；先做八度分离，再统一移调，A4=440 Hz，不增加失谐叠音。原谱的延音踏板以合成器有限的释放尾音代替，让高音旋律的快速起音保持清楚。
