export interface UIStatus {
  ...
  beginLoading: (fast_load?: boolean) => this;
  endLoading: () => this;
  setLoadProgress: (progress: number, relative?: boolean) => this;
  back: () => this;
}
...
export function createUIStatusStatic(beginPage: string): UIStatus {
  return <UIStatus>{
    ...
    setLoadProgress(progress: number, relative: boolean = false): UIStatus {
      const new_progress = relative
        ? progress
        : this.__load_progress + progress;
      this.__load_progress =
        new_progress < 0 ? 0 : new_progress > 100 ? 100 : new_progress;
      return this;
    },
    back() {
...
export function createNewAnimationData(
  ...
  return {
    ...
    get duration() {
      if (this.__max_duration && this.__raw_duration > this.__max_duration)
        return this.__max_duration;

      if (this.__min_duration && this.__raw_duration < this.__min_duration)
        return this.__min_duration;
      return this.__raw_duration;
    },
    setFactor(factor: number) {
      if (factor < 0)
        throw new Error("Animation duration factor cannot be negative.");
    ...
    setMinDuration(duration: number) {
      ...
      if (this.__max_duration && duration > this.__max_duration)
        throw new Error(
          "Minimum animation duration cannot be bigger than maximum animation duration.",
        );
    ...
    setMaxDuration(duration: number) {
      ...
      if (duration < this.__min_duration)
        throw new Error(
          "Maximum animation duration cannot be smaller than minimum animation duration.",
        );
